...
will run the actual scanner at tools/momentum_spike.py.
...
def _sanitize_env_number(name: str, default: str) -> None:
    """Strip inline comments and commas from numeric env vars."""
    raw = os.getenv(name, default)
    raw = raw.split("#", 1)[0].replace(",", "").strip()
    os.environ[name] = raw

# Common knobs to sanitize (add more if you use them)
...
# Ensure tools/ is importable and execute the real script
