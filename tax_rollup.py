...
def main():
    ...
    by_year = defaultdict(lambda: {"profit_usd": 0.0, "reserved_usd": 0.0})
    with open(LEDGER, "r", newline="", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for r in reader:
            y = r.get("year") or "unknown"
            try:
                by_year[y]["profit_usd"] += float(r.get("profit_usd", "0") or 0)
                by_year[y]["reserved_usd"] += float(r.get("reserved_usd", "0") or 0)
            except Exception:
                pass
    ...
    for y, agg in by_year.items():
        ...
        with open(outp, "w", newline="", encoding="utf-8") as f:
            ...
            w.writerow([y, f"{agg['profit_usd']:.2f}", f"{agg['reserved_usd']:.2f}"])
        print(f"Wrote {outp}")
