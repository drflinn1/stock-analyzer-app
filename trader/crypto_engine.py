...
def _b(s, d):
    return str(s).lower() in {"1", "true", "yes", "on"} if s is not None else d
...
def fetch_positions_snapshot(ex: ccxt.Exchange):
    ...
    for sym, m in ex.markets.items():
        ...
        pos.append(
            {"symbol": sym, "base": base, "qty": qty, "price": price, "usd_value": qty * price}
        )
    return pos
...
def ensure_entry(ledger: Dict[str, Dict], symbol: str, entry: float, set_high: bool = True):
    rec = ledger.get(symbol.upper())
    if not rec:
        ledger[symbol.upper()] = {
            "entry": float(entry),
            "high": float(entry if set_high else 0.0),
            "added": _now(),
        }
    else:
...
def _backfill_avg_entry_from_trades(
    ex: ccxt.Exchange, symbol: str, current_qty: float, lookback_days: int
):
    """
...
def run_trading_loop() -> int:
    ...
    if WL:
        universe = [c for c in universe if c["symbol"].upper() in WL]
    filtered = [
        c
        for c in universe
    ...
    for c in filtered:
        if buys >= slots or spendable < MIN_BUY_USD:
            break
        sym = c["symbol"].upper()
        try:
            buy_market(ex, sym, MIN_BUY_USD, dry)
            last = float(ex.fetch_ticker(sym)["last"])
            ensure_entry(ledger, sym, last, set_high=True)
            save_ledger(ledger)
            buys += 1
    ...
    if buys == 0 and sells == 0:
        ...
        if buys:
            print(f"[engine] Buys executed: {buys}")
        if sells:
            print(f"[engine] Sells executed: {sells}")
    return 0
...
def run_hourly_rotation(
    ...
    The engine reads env vars inside run_trading_loop(); kwargs are ignored.
    """
