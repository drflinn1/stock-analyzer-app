...
def env_str(name: str, default: str = "") -> str:
    v = os.getenv(name); return default if v is None else str(v).strip()
    ...
    if path.exists():
        ...
        with path.open() as f:
            ...
            for i, h in enumerate(header):
                if h.strip().lower() == "symbol": sym_idx = i; break
            if sym_idx is None:
                sym_idx = 0
                if len(header) > 0: out.append(str(header[sym_idx]).strip())
            for row in r:
                if row: out.append(str(row[sym_idx]).strip())
...
def run_hourly_rotation() -> None:
    ...
    cands = [_fmt(c) for c in load_candidates() if c]
    if not cands:
        write_summary({"when": when_utc, "dry_run": "OFF" if not dry_run else "ON",
                       "buy_usd": str(buy_usd), "tp_pct": env_str("TP_PCT","5"),
                       "stop_pct": env_str("STOP_PCT","1"), "window_min": env_str("WINDOW_MIN","30"),
                       "slow_gain_req": env_str("SLOW_GAIN_REQ","3"), "universe_pick": "AUTO",
                       "status": "noop",
                       "note": "No candidates available (WHITELIST empty and momentum_candidates.csv not found)."})
        LAST_EXIT.write_text("0"); return

    gates: List[Dict[str, Any]] = []
    picked = None; final_amount = 0.0; final_cost = 0.0

    for sym in cands:
        price = _fetch_price(ex, sym)
        if not price or price <= 0:
            gates.append({"pair": sym, "price": 0.0, "min_amount": 0.0, "min_cost": 0.0,
                          "buy_usd": buy_usd, "decision": "Skip — no price"}); continue
        if min_price > 0 and price < min_price:
            gates.append({"pair": sym, "price": price, "min_amount": 0.0, "min_cost": 0.0,
                          "buy_usd": buy_usd, "decision": f"Skip — price ${price:.8f} < MIN_PRICE_USD"}); continue

        lim = _pair_limits(ex, sym, price)
        min_amt, min_cost = lim["min_amount"], lim["min_cost"]
        intended_amount = buy_usd / price

        if intended_amount >= min_amt and buy_usd >= min_cost:
            picked = sym; final_amount = intended_amount; final_cost = buy_usd
            gates.append({"pair": sym, "price": price, "min_amount": min_amt, "min_cost": min_cost,
                          "buy_usd": buy_usd, "decision": f"OK — will buy ~{final_amount:.8f} for ${final_cost:.2f}"}); break

        bumped_amount = max(intended_amount, min_amt)
        bumped_cost = bumped_amount * price
        if bumped_cost <= buy_usd:
            picked = sym; final_amount = bumped_amount; final_cost = bumped_cost
            gates.append({"pair": sym, "price": price, "min_amount": min_amt, "min_cost": min_cost,
                          "buy_usd": buy_usd, "decision": f"Auto-bump — buy min {final_amount:.8f} (${final_cost:.2f})"}); break
    ...
    try:
        ...
        POS_FILE.write_text(json.dumps({"pair": picked, "amount": final_amount,
                                        "est_cost": round(final_cost, 8), "when": when_utc}, indent=2))
        status = "ok"
    except ccxt.BaseError as e:
        status = "error"; note = f"InvalidOrder: kraken {str(e)}"
