...
Triggers:
- STOP_PCT: sell if drop >= STOP_PCT since entry (anytime).
...
def run_sell_guard(
    *, dry_run: Optional[bool]=None, tp_pct: Optional[float]=None, stop_pct: Optional[float]=None,
    window_min: Optional[int]=None, slow_gain_req: Optional[float]=None, **_
) -> Dict[str, Any]:
    ...
    pair   = pos.get("pair"); 
    amount = float(pos.get("amount",0)); est_cost = float(pos.get("est_cost",0))
    if not pair or amount<=0 or est_cost<=0:
        return {"action":"hold","note":"Position file incomplete; skipping sell."}

    entry = est_cost/amount
    opened = _parse_when(pos.get("when","")) or datetime.now(timezone.utc)
    age_min = max(0.0, (datetime.now(timezone.utc)-opened).total_seconds()/60.0)

    ex, have_keys = _kraken()
    try:
        t = ex.fetch_ticker(pair)
        price = float(t.get("last") or t.get("close") or t.get("bid") or t.get("ask"))
    except Exception:
        return {"action":"hold","note":f"Could not fetch price for {pair}; holding."}

    pct = (price/entry - 1.0)*100.0
    reason = None
    if pct <= -abs(sl):          reason = f"STOP {sl:.2f}% hit"
    elif pct >= abs(tp):         reason = f"TP {tp:.2f}% hit"
    elif age_min >= float(win) and pct < slow_req: reason = f"Slow gain (<{slow_req:.2f}% after {win}m)"

    if not reason:
        return {"action":"hold","note":f"Holding {pair}: {pct:+.2f}% (age {age_min:.1f}m)."}

    mode = "DRY" if (dry_run or not have_keys) else "LIVE"; order_id="-"
    try:
        if mode=="LIVE":
        ...
        _append_summary_note(note); notify_slack(note)
