...
- Scans a liquid universe, ranks by momentum/trend, auto-picks top names.
- TREND entries (EMA12>EMA26 & RSI>50), TREND exits (EMA12<EMA26 & RSI<50).
- Respects PER_TRADE_USD and DAILY_CAP_USD. Skips rebuy if already holding.

New:
  MIN_ADV_USD (default 5,000,000) controls liquidity threshold using IEX volume.
...
Env knobs:
  ...
  PICKS_PER_RUN=4
  UNIVERSE=...              # used only if AUTO_PICK=false
  SCAN_UNIVERSE=...         # optional; default list provided
...
def getenv_bool(k: str, d: bool) -> bool:
    v = os.getenv(k);  return d if v is None else str(v).lower().strip() in {"1","true","yes","y"}
...
MIN_ADV_USD   = float(os.getenv("MIN_ADV_USD", "5000000"))  # <<< lowered for IEX feed
...
def fetch_daily_bars_alpaca(client: Optional[StockHistoricalDataClient], symbol: str, days: int) -> pd.DataFrame:
    ...
    req = StockBarsRequest(symbol_or_symbols=symbol, timeframe=TimeFrame.Day,
                           start=start, end=end, adjustment="raw", feed="iex", limit=None)
    try:
        bars = client.get_stock_bars(req); df = bars.df
        if df is None or df.empty: return pd.DataFrame()
        if isinstance(df.index, pd.MultiIndex):
            df = df.xs(symbol, level=0, drop_level=True)
        df = df.reset_index().rename(columns={"timestamp":"date"})
        df = df[["date","open","high","low","close","volume"]]
        df.sort_values("date", inplace=True); df.set_index("date", inplace=True); return df
    except Exception as e:
        log.error(f"Alpaca data error for {symbol}: {e}"); return pd.DataFrame()

def fetch_daily_bars_yf(symbol: str, days: int) -> pd.DataFrame:
    if not HAVE_YF: log.error("yfinance not installed and Alpaca data failed."); return pd.DataFrame()
    try:
        ...
        log.error(f"yfinance error for {symbol}: {e}"); return pd.DataFrame()
...
# trading
@dataclass
class Position: symbol: str; qty: float
...
def trend_signal(df: pd.DataFrame) -> str:
    ...
    if ema12_now > ema26_now and r_now > 50: return "BUY"
    if ema12_now < ema26_now and r_now < 50: return "SELL"
    return "HOLD"
...
def main() -> int:
    ...
    log.info(f"Config → DRY_RUN={DRY_RUN} PER_TRADE_USD={PER_TRADE_USD} DAILY_CAP_USD={DAILY_CAP_USD} "
             ...
             f"AVOID_REBUY={AVOID_REBUY} MIN_ADV_USD={MIN_ADV_USD:,.0f}")
    ...
    if AUTO_PICK:
        ...
        for sym in SCAN_UNIVERSE:
            ...
            if s == -1e9:  # liquidity/history fail already logged
                continue
            if trend_signal(df) != "BUY":  # must be bullish TREND to qualify
                continue
            scored.append((sym, s, n)); time.sleep(0.05)
        ...
        if not working:
            log.info("No eligible picks; exiting without trades."); return 0
    else:
        working = UNIVERSE; log.info(f"Using provided UNIVERSE: {working}")

    spend_left = DAILY_CAP_USD; total_buys = 0; total_sells = 0
