...
DROP_PCT_GATE        = float(os.getenv("DROP_PCT", "2.5"))
...
SIM_BALANCE_QUOTE = os.getenv("SIM_BALANCE_QUOTE")  # e.g., "100" to simulate DRY cash
...
def add_daily_spend(state, usd): state["spend"][today_key()] = float(state["spend"].get(today_key(), 0.0) + float(usd))
def spent_today(state): return float(state["spend"].get(today_key(), 0.0))
...
def init_exchange():
    global EX
    if EX is None: EX=kraken_client()

def price(symbol:str)->float:
    init_exchange()
    t=EX.fetch_ticker(symbol)
    last=t.get("last") or t.get("close")
    if last is None: raise RuntimeError(f"No last price for {symbol}")
    return float(last)
...
def change_pct_15m(symbol:str)->float:
    ...
    if ohlcv and len(ohlcv)>=2:
        return pct(float(ohlcv[-1][4]), float(ohlcv[-2][4]))
    t=EX.fetch_ticker(symbol); o=t.get("open"); l=t.get("last") or t.get("close")
...
def base(sym:str)->str: return sym.split("/")[0]
def quote(sym:str)->str: return sym.split("/")[1] if "/" in sym else "USD"
def qty_free(sym:str)->float:
    bal=balances(); return float(bal.get("free",{}).get(base(sym),0.0))

def free_balance_for_quote(q:str)->float:
    bal=balances(); free=bal.get("free",{}) or {}
    candidates=[q,q.upper()]
    if len(q)==3: candidates += [f"Z{q.upper()}", f"X{q.upper()}"]  # Kraken fiat aliases
    for k in candidates:
        if k in free:
...
def place_buy(symbol:str, quote_amount:float, px:float):
    amt=max(quote_amount/px,0.0); amt=float(f"{amt:.6f}")
    if amt<=0: print(f"[SKIP] BUY {symbol} amount<=0"); return {"status":"skip"}
    if DRY_RUN: print(f"[DRY] BUY {symbol} ${fmt2(quote_amount)} @ {fmt2(px)} (≈{amt})"); return {"status":"dry","symbol":symbol,"filled":amt}
    init_exchange(); o=EX.create_order(symbol=symbol,type="market",side="buy",amount=amt)
    print(f"[LIVE] BUY {symbol} market amount={amt} (order {o.get('id')})"); return {"status":"ok","symbol":symbol,"filled":amt,"order":o}

def place_sell(symbol:str, qty:float, px:float):
    qty=float(f"{qty:.6f}")
    if qty<=0: print(f"[SKIP] SELL {symbol} qty<=0"); return {"status":"skip"}
    if DRY_RUN: print(f"[DRY] SELL {symbol} qty={qty:.6f} @ {fmt2(px)}"); return {"status":"dry","symbol":symbol,"filled":qty}
    init_exchange(); o=EX.create_order(symbol=symbol,type="market",side="sell",amount=qty)
    print(f"[LIVE] SELL {symbol} market qty={qty:.6f} (order {o.get('id')})"); return {"status":"ok","symbol":symbol,"filled":qty,"order":o}
...
def compute_rsi(closes,length:int)->float:
    ...
    for i in range(length+1,len(closes)):
        ch=closes[i]-closes[i-1]; g=max(ch,0.0); l=max(-ch,0.0)
        avg_gain=(avg_gain*(length-1)+g)/length; avg_loss=(avg_loss*(length-1)+l)/length
...
def ensure_entry(state,symbol,entry_avg):
    ...
    if "entry_avg" not in pos or pos["entry_avg"]<=0: pos["entry_avg"]=float(entry_avg)
    pos.setdefault("peak_pnl_pct",0.0); pos.setdefault("trail_active",False)

def maybe_activate_trailing(state,symbol,pnl_pct):
    pos=state["positions"][symbol]
    if pnl_pct>pos["peak_pnl_pct"]: pos["peak_pnl_pct"]=float(pnl_pct)
    if not pos["trail_active"] and pnl_pct>=TRAIL_START_PCT: pos["trail_active"]=True

def should_trail_exit(state,symbol,pnl_pct):
    pos=state["positions"][symbol]
    if not pos["trail_active"]: return False
...
def run_once():
    ...
    state=load_state(); buys_placed=0; sells_placed=0

    # SELL checks
    for sym in SYMBOLS:
        qty=qty_free(sym)
        if qty<=0: continue
        px=price(sym); ensure_entry(state,sym,entry_avg=px)
        entry_avg=state["positions"][sym]["entry_avg"]; pnl_pct=pct(px,entry_avg)
        maybe_activate_trailing(state,sym,pnl_pct)
        reason=decide_sell(pnl_pct)
        if reason is None and should_trail_exit(state,sym,pnl_pct): reason="TRAIL"
        log=f"{now_iso()} | {sym} | entry_avg={fmt2(entry_avg)} | last={fmt2(px)} | PnL={('+' if pnl_pct>=0 else '')}{fmt2(pnl_pct)}% "
        if state["positions"][sym]["trail_active"]: log+=f"| trail_on peak={fmt2(state['positions'][sym]['peak_pnl_pct'])}% "
        if reason: log+=f"| SELL ({reason})"
        print(log)
        if reason:
            place_sell(sym,qty,px); sells_placed+=1; state["positions"].pop(sym,None)
    ...
    if can_spend>=max(MIN_BALANCE_USD,POSITION_SIZE_USD) and open_trades<MAX_OPEN_TRADES:
        ...
        for sym,chg,rsi,ok in sorted(candidates,key=lambda x:x[1]):
            if ok: picked=(sym,chg,rsi); break
        if picked:
            psym,pchg,prsi=picked; px=price(psym)
            ...
            if r.get("status") in ("ok","dry"):
                buys_placed+=1; add_daily_spend(state,POSITION_SIZE_USD); ensure_entry(state,psym,entry_avg=px)
        else:
            if candidates:
