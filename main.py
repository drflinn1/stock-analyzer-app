...
def as_int(s, default): 
    try: return int(float(s)) if s not in (None,"") else default
    except: return default
def now_iso(): 
...
def map_base_to_usd_symbol(ex, base: str) -> str | None:
    ...
    if base in ("USD","USDT","USDC"): return None
    cands = ["XBT/USD"] if base=="BTC" else []
...
def load_screener_or_compute(ex):
    ...
    rows=[]; skip={"USDT","USDC","USD","EUR","GBP"}
    for m in ex.markets.values():
        if not m.get("spot"): continue
        if m.get("quote")!="USD": continue
        base=m.get("base",""); if base in skip: continue
        sym=m["symbol"]; t=fetch_ticker_safe(ex,sym)
        rows.append({"symbol":sym,"pct":pct24(t) or -9999,"last":last_price(t),"spread":spread_pct(t),"qvol":t.get("quoteVolume") or 0.0})
    rows.sort(key=lambda r:r["pct"], reverse=True)
    top=rows[:12]; fp.write_text(json.dumps(top, indent=2)); return top
...
def portfolio_snapshot(ex):
    ...
    if HAS_KEYS:
        ...
        for base,qty in totals.items():
            if not qty: continue
            b=str(base).upper()
            if b in ("USD","USDT","USDC"): continue
            sym=map_base_to_usd_symbol(ex,b); 
            if not sym: continue
            t=fetch_ticker_safe(ex,sym); p=last_price(t) or 0.0
            usd=p*float(qty); if usd<=0: continue
            holds.append({"base":b,"symbol":sym,"qty":float(qty),"usd_value":float(usd),"price":p,"pct24":pct24(t) if pct24(t) is not None else -9999.0})
...
def load_positions(): 
    if POS_FILE.exists():
        try: return json.loads(POS_FILE.read_text())
        except: pass
    return {}
def save_positions(d): POS_FILE.write_text(json.dumps(d, indent=2))
def pos_init_if_missing(positions, sym, qty, price):
    if sym not in positions: positions[sym]={"entry":price,"high":price,"qty":qty,"tp1_done":False,"updated":now_iso()}
def pos_apply_buy(positions, sym, buy_qty, buy_price):
    p=positions.get(sym)
    if not p: positions[sym]={"entry":buy_price,"high":buy_price,"qty":buy_qty,"tp1_done":False,"updated":now_iso()}; return
    new_qty=p["qty"]+buy_qty
    if new_qty<=0: positions.pop(sym,None); return
    p["entry"]=(p["entry"]*p["qty"]+buy_price*buy_qty)/new_qty
    p["qty"]=new_qty; p["high"]=max(p["high"],buy_price)
    if buy_price < p["entry"]: p["tp1_done"]=False
    p["updated"]=now_iso()
def pos_apply_sell(positions, sym, sell_qty):
    p=positions.get(sym); if not p: return
    rem=p["qty"]-sell_qty
    if rem<=1e-12: positions.pop(sym,None)
    else: p["qty"]=rem; p["updated"]=now_iso()
...
def market_buy_usd(ex, symbol, usd_amount, dry=True):
    ...
    t=fetch_ticker_safe(ex,symbol); price=last_price(t)
    if not price or price<=0: raise RuntimeError(f"No price for {symbol}")
    amount=usd_amount/price; amount=float(ex.amount_to_precision(symbol, amount))
    if amount<=0: raise RuntimeError(f"Amount too small for {symbol}")
    if dry:
...
def market_sell_all(ex, symbol, qty, reason, dry=True):
    ...
    if qty<=0: raise RuntimeError(f"Sell qty too small for {symbol}")
    tag=reason.upper()
    if dry:
...
def sweep_dust(ex, snap):
    if not HAS_KEYS: 
        # nothing to sweep in simulated mode
        return []
    swept=[]
    for h in snap["holds"]:
    ...
    if swept: print(yellow(f"Swept dust: {', '.join(swept)}"))
    return swept

def manage_protection(ex, snap, positions):
    if not HAS_KEYS:
        # No real holdings to protect in simulated mode
        return
    actions=[]
    for h in snap["holds"]:
        ...
        entry=p["entry"]; high=p["high"]; tp1_done=p.get("tp1_done",False)
        sl_price=entry*(1-SL_PCT); trail_price=high*(1-TRAIL_PCT); tp1_price=entry*(1+TP1_PCT)
        if price <= sl_price:
            actions.append(("STOP_LOSS", sym, qty)); continue
        if high > entry and price <= trail_price:
            actions.append(("TRAIL", sym, qty)); continue
        if (not tp1_done) and price >= tp1_price and TP1_SIZE>0:
            actions.append(("TAKE_PROFIT", sym, float(qty*TP1_SIZE)))
    for act in actions:
        kind, sym, qty = act[0], act[1], act[2]
        try:
...
def plan_rotation(ex, screener, snap):
    holds=snap["holds"]; cash=snap["cash_usd"]
    ...
    for r in screener:
        ...
        sp=r.get("spread")
        if isinstance(sp,(int,float)) and sp>SPREAD_MAX_PCT: continue
        m=ex.markets[sym]; min_cost=((m.get("limits") or {}).get("cost") or {}).get("min")
        if isinstance(min_cost,(int,float)) and min_cost and min_cost>MIN_NOTIONAL_USD: continue
        filt.append(r)
    filt.sort(key=lambda r:r.get("pct") or -9999, reverse=True)
    to_buy=[]; to_sell=[]; notes=[]
    slots_left=max(0, MAX_POSITIONS - len(holds)); buys_left=MAX_BUYS_PER_RUN
    ...
    if (len(holds)>=MAX_POSITIONS) or (cash<MIN_TRADE_USD):
        if filt:
            cand=filt[0]; cand_pct=cand.get("pct") or -9999
            worst=None
            for h in holds:
                if worst is None or h["pct24"] < worst["pct24"]: worst=h
...
def main():
    ...
    sweep_dust(ex, snap)
    manage_protection(ex, snap, positions); save_positions(positions)
    snap=portfolio_snapshot(ex)

    plan=plan_rotation(ex, screener, snap); (STATE/"last_plan.json").write_text(json.dumps(plan, indent=2))
    for n in plan["notes"]: print(yellow(f"NOTE: {n}"))

    for s in plan["to_sell"]:
        try:
            market_sell_all(ex, s["symbol"], s["qty"], "ROTATE", DRY_RUN); pos_apply_sell(positions, s["symbol"], s["qty"])
        except Exception as e: print(red(f"Rotate sell error {s['symbol']}: {e}"))

    for b in plan["to_buy"]:
        try:
            res=market_buy_usd(ex, b["symbol"], b["usd"], DRY_RUN)
            if res.get("price") and res.get("amount"): pos_apply_buy(positions, b["symbol"], float(res["amount"]), float(res["price"]))
        except Exception as e: print(red(f"Buy error {b['symbol']}: {e}"))

    save_positions(positions)
    # KPI
    kpi=STATE/"kpi_history.csv"; write_header=not kpi.exists()
    with kpi.open("a", newline="") as f:
        w=csv.writer(f)
        if write_header: w.writerow(["utc","dry_run","cash_usd","n_holds","sl","trail","tp1","tp1_size"])
        w.writerow([now_iso(), DRY_RUN, snap["cash_usd"], len(snap["holds"]), SL_PCT, TRAIL_PCT, TP1_PCT, TP1_SIZE])

if __name__=="__main__":
    try: main()
    except Exception as e:
        print(red(f"FATAL: {e}")); raise
