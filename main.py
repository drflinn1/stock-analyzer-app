...
- Features:
  ...
  * Momentum entries with rotation when cash is short
  * Daily notional cap + per-symbol rebuy cooldown
  * Dust sweep under DUST_MIN_USD
  * STOP_LOSS / TAKE_PROFIT (TP1) / Trailing exits on each run
...
# ---- Sell guards (keep names for guard workflow) ----
...
TP1_SIZE            = env_float("TP1_SIZE", 0.25)   # size to sell at TP1
...
# -------------------------- Exit Logic (STOP_LOSS / TAKE_PROFIT / Trailing / Dust) -------------------------- #

def evaluate_exits(ex, snapshot, coin_balances) -> Tuple[float, float]:
    """Returns (usd_from_sells, usd_dusted)"""
    usd_realized = 0.0
    usd_dusted = 0.0
    for symbol in WHITELIST:
        ...
        if value < DUST_MIN_USD:
            ...
            update_after_sell(symbol, sqty, spx)
            usd_dusted += sqty * spx
            continue
...
# -------------------------- Entries (Momentum + Rotation + Caps) -------------------------- #
...
def pick_candidates(snapshot, held_symbols: List[str]) -> List[str]:
    ...
    ordered = [s for (s,_) in scored if s not in held_symbols] + [s for (s,_) in scored if s in held_symbols]
    return ordered
...
def main():
    ...
    # sync positions qty with balances
    for s in WHITELIST:
        base = s.split("/")[0]
    ...
    print(f"[SUMMARY] PV≈${portfolio_value:.2f} | USD≈${usd_free:.2f}")
