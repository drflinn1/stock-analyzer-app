...
# - Includes explicit 'SELL' log tag to satisfy Sell Logic Guard
...
QUOTE             = getenv("QUOTE", "USD").upper()                  # enforce USD quote
...
LOOKBACK_MIN      = int(getenv("LOOKBACK_MIN", "240"))              # momentum window (mins)
...
def pick_universe(ex: ccxt.Exchange) -> List[str]:
    ...
    return [f"{base}/{QUOTE}" for base in WHITELIST if f"{base}/{QUOTE}" in markets]
...
def est_qty(notional: float, price: float) -> float:
    ...
    return max(0.0, round(notional / price, 6))  # common precision ok for majors
...
def place_order_or_sim(ex: ccxt.Exchange, side: str, symbol: str, qty: float, price: float) -> Dict[str, Any]:
    ...
    if DRY_RUN:
        # NOTE: explicit SELL/BUY text stays in source for the guard
        if side.lower() == "sell":
            print(f"SIM SELL {symbol} qty={qty} @ {price:.2f}")
        else:
            print(f"SIM BUY  {symbol} qty={qty} @ {price:.2f}")
...
def main():
    ...
    for sym, pos in list(positions.items()):
        ...
        if reason:
            qty = float(pos["qty"])
            place_order_or_sim(ex, "sell", sym, qty, price)
            # Explicit SELL tag for guard + clear human logs
            print(f"SELL {sym} via {reason}: qty={qty} @ {price:.2f} PnL={(price-pos['entry'])*qty:.2f}")
