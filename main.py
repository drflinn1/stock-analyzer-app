#!/usr/bin/env python3
# main.py — Auto-Universe + Trailing Profit + "skip if per-trade < exchange min"
#
# New behavior:
# - Before buying, estimate each pair's minimum notional (from CCXT limits or
#   amount_min * price + small fee buffer). If PER_TRADE_USD is below that,
#   we SKIP with a clear log instead of throwing "Insufficient funds".
#
# New logs you may see:
#   auto_universe: picked N of M candidates -> [...]
#   SKIP buy ETH/USD: per-trade $25.00 < estimated minimum $22.70 (incl. buffer)
#   SKIP buy BTC/USD: insufficient free USD ($18.43 < $25.00)
#   TRAIL activate ..., TRAIL new high ..., SELL placed ... (unchanged)
...
# Manual fallback if AUTO_UNIVERSE=false
...
# ── Estimate exchange minimum notional (USD) ───────────────────────────────────
def estimate_min_cost_usd(ex, symbol: str, price_now: float) -> float:
    """
    Try to estimate the minimum notional the exchange accepts for this symbol.
    Priority: explicit cost.min -> amount.min * price -> 1 lot (10^-amount_prec) * price.
    Add a small fee buffer (0.5%) and 1c headroom.
    """
...
# Optional: check free USD when live (best-effort; safe if it fails)
def get_free_usd(ex) -> float:
    try:
        bal = ex.fetch_balance()
        # CCXT balance structures vary; try both ways.
        ...
        return float("inf")  # don't block if we can't fetch
...
def pick_universe(ex) -> List[str]:
    ...
    for s, m in ex.markets.items():
        ...
        try:
            ...
            last = 0.0
        if MIN_PRICE_USD > 0 and (last <= 0 or last < MIN_PRICE_USD): continue
        # Quote volume proxy
        qvol = 0.0
        try:
            t = ex.fetch_ticker(s)
...
def place_market_buy(ex, symbol: str, usd_amount: float, price_now: float, positions: Dict[str, Any]):
    ...
    # Check pair minimum notional (estimated)
    ...
    # Optional: check free USD when live
    if not DRY_RUN:
    ...
    amt_prec, _pp, _min_cost, _m = _market_precision(ex, symbol)
