# main.py — Atomic rotation, 24h% ranking, weakest-eligible first, WHY-NOT logs
#
# Features / Knobs (env):
#   EXCHANGE="kraken"
...
#
#   RESERVE_CASH_USD="25"           # (informational; rotation budgets use USD_PER_TRADE)
#   USD_PER_TRADE="10"
#   MIN_COST_PER_ORDER="5.0"
#   MAX_SPREAD_BPS="75"
#   COOLDOWN_MINUTES="10"
#
#   UNIVERSE=""                     # empty = autodetect USD spot markets
#   IGNORE_TICKERS="USDT,USDC,USD,EUR,GBP,XRP"
#
#   ROTATE_WEAKEST_STRICT="false"   # if weakest can't be sold, skip rotation
#   PARTIAL_ASSIST="true"           # (kept for parity; not used in atomic mode)
#   WHY_NOT_LOGS="true"
#
#   TAKE_PROFIT_PCT="0.05"          # TAKE_PROFIT guard (uses 24h% proxy)
#   STOP_LOSS_PCT="0.03"            # STOP_LOSS guard
#   TRAILING_STOP_PCT="0.02"        # TRAIL drawdown guard
#   EXIT_SCOPE="weakest"            # weakest | bottom | all
#   EXIT_BEFORE_ROTATE="false"      # default: rotation first
#
#   ATOMIC_ROTATION="true"          # require buy+sell both eligible
#   MIN_QUOTE_VOL_USD="50000"       # filter thin markets from Top-K
#
#   SELL_EPS="0.995"                # sell <=99.5% to avoid fee/precision rejects
#   BUY_EPS="0.995"                 # spend <=99.5% of budget
#
...
# Exit guards (CI scan tokens preserved)
...
# Rotation safety & universe quality
...
def score_from_ticker(t: dict) -> float:
    """Robust 24h return: prefer exchange 'percentage', else (last-open)/open."""
    ch = t.get("percentage")
    if ch is not None:
        try:
...
def main() -> int:
    ...
    # ----- universe: USD spot, active, min vol, spread filter -----
    ...
    print("[RANK] Top candidates (with 24h%):")
    for c in ranked[:TOP_K]:
        t = tickers.get(f"{c}/USD") or {}
        print(f"  - {c:<8} {scores[c]:+6.2%}")
    top = ranked[:max(TOP_K,1)]
    print(f"[RANK] Top {TOP_K}: {top}")

    # ----- choose weakest: first *sell-eligible* loser among holdings -----
    ...
    # ----- scan Top-K for the BEST eligible buy by 24h% -----
    usd_to_use = max(USD_PER_TRADE, MIN_COST_PER_ORDER) * BUY_EPS
    buy = None; btkr = None; best_score = -1e9
    ...
    for c in top:
        ...
        if vol_q is None:
            ...
            vol_q = (bv or 0) * (px or 0)
        why=[]
        ok = buy_eligible(ex, c, usd_to_use, t, why)
        flag = "OK" if ok else "NO"
        print(f"  - {c:<8} 24h={s:+6.2%}  spread={bps:>4.0f} bps  volUSD≈{(vol_q or 0):>9,.0f}  {flag} {'; '.join(why)}")
        if ok and s > best_score:
            best_score, buy, btkr = s, c, t
    ...
    edge = (scores.get(buy, 0.0) - scores.get(weakest, 0.0)) if buy else 0.0
    ...
    if EXIT_BEFORE_ROTATE:
        ...
        did_anything |= do_rotation(ex, weakest, w_amt, wtkr, buy, btkr, usd_to_use, edge)
    else:
        did_anything |= do_rotation(ex, weakest, w_amt, wtkr, buy, btkr, usd_to_use, edge)
        did_anything |= do_exits(ex, holdings, free, tickers, top, weakest)
...
# ---------- rotation ----------
def do_rotation(ex, weakest, w_amt, wtkr, buy, btkr, usd_to_use, edge) -> bool:
    if buy is None:
        print("[ROTATE] No buy target; skipping sell.")
        return False
    ...
    # SELL weakest (precision clamp + one retry smaller)
    dec = amount_precision(ex, f"{weakest}/USD")
    sell_amt = clamp(max(w_amt, 0.0), dec)
    print(f"[ROTATE] SELL weakest {weakest} amount={sell_amt}")
    if DRY_RUN:
        print(f"[DRY-RUN] create_order SELL {weakest}/USD amount={sell_amt}")
        did = True
    else:
        try:
            ex.create_order(symbol=f"{weakest}/USD", type="market", side="sell", amount=sell_amt)
            save_last_trade(weakest); did = True
        except Exception as e:
            print(f"[ERROR] sell failed ({weakest}): {e}")
            retry_amt = clamp(sell_amt * 0.97, dec)
            if retry_amt > 0 and retry_amt != sell_amt:
                print(f"[RETRY] SELL {weakest} amount={retry_amt}")
                try:
                    ex.create_order(symbol=f"{weakest}/USD", type="market", side="sell", amount=retry_amt)
                    save_last_trade(weakest); did = True
...
def do_exits(ex, holdings, free, tickers, top, weakest) -> bool:
    ...
    for sym in sorted(symbols):
        ...
        do_tp = take_profit_check(sym, t)
        do_sl = stop_loss_check(sym, t)
        do_tr = trailing_stop_check(sym, t)
        ...
        dec = amount_precision(ex, f"{sym}/USD")
        amt = clamp(amt, dec)
