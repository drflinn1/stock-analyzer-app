...
def analyze(df: pd.DataFrame) -> dict | None:
    if len(df) < 30:  # lowered from 60 to 30
        if debug_mode:
...
def log_trade(tkr, summ, price):
    if summ["Signal"] == "HOLD": return
    row = {
        "Date":datetime.now().isoformat(), "Ticker":tkr,
        "Signal":summ["Signal"], "Price":price,
...
if st.button("▶ Run Analysis"):
    ...
    for tkr in tickers:
        try:
            ...
            if debug_mode: st.write(f"{tkr}: {len(df)} rows")
            summ = analyze(df)
